...
ユーザーが /discuss コマンドを投げると、システムが議題ごとの専用スレッドを自動生成し、Dify経由で複数のローカルAI（Gemma, Phi, Llama）を呼び出して議論させ、最後にGeminiがまとめます。
...
* URLのクエリ文字列 ?bot=... (例: bot=gemini, bot=llama) を取得してください。  
...
* 検知したIDをマッピング配列（例: ['1111111' => 'Llama', '2222222' => 'Gemma']）と照合し、次に発言すべき $targetAi を特定してください。  
